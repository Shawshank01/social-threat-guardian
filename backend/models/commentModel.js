...
export async function fetchLatestComments(limit = 4, filters = {}) {
  ...
  const predIntent = filters.predIntent ? String(filters.predIntent).trim() : null;

  return withConnection(async (conn) => {
    const binds = { limit: capped };
    if (predIntent) binds.predIntent = predIntent;

    const filterClause = predIntent ? "WHERE PRED_INTENT = :predIntent" : "";
    ...
    const result = await conn.execute(
      `
        SELECT POST_TEXT, PRED_INTENT, POST_TIMESTAMP
          FROM (
            SELECT POST_TEXT, PRED_INTENT, POST_TIMESTAMP
              FROM HATE_SPEECH_DUMMY_RESULTS
             ${filterClause}
